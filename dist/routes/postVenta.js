...
router.put('/clientes/:id', async (req, res) => {
    try {
        const updatedCliente = await ClientePostVenta.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!updatedCliente) {
            return res.status(404).json({ message: 'Cliente no encontrado' });
        }
...
router.put('/equipos/:id', async (req, res) => {
    try {
        const updatedEquipo = await EquipoPostVenta.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!updatedEquipo) {
            return res.status(404).json({ message: 'Equipo no encontrado' });
        }
...
//# sourceMappingURL=postVenta.js.map